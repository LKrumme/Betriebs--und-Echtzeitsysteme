...
// Not using spi_complete, since interrupts are disabled here
// Note: This leads to the display showing weird behavior in
// debug mode
/*void spi_complete()
{
...
void mode_data(void)
{
	...
	if(!(GPIO_REG(GPIO_OUTPUT_VAL) & (1 << mapPinToReg(OLED_DC))))
	{
		...
		GPIO_REG(GPIO_OUTPUT_VAL) |= 1 << mapPinToReg(OLED_DC);
	}
}
...
void oled_init()
{
	...
	//RESET
	GPIO_REG(GPIO_OUTPUT_VAL) &= ~(1 << mapPinToReg(OLED_RES));
	sleep_u(10);		//min 10us
	GPIO_REG(GPIO_OUTPUT_VAL) |= 1 << mapPinToReg(OLED_RES);
